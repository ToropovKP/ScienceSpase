...
import { CoAuthorsFormArrayComponent } from '../../../features/podium/co-authors/ui/co-authors-form-array.component';
import { JobFilesManagerComponent } from '../../../features/podium/job-files/ui/job-files-manager.component';
import { JobContactInfoFormComponent } from '../../../features/podium/job-contact-info/ui/job-contact-info-form.component';
import { JobContextSelectorComponent } from '../../../features/podium/job-context-selector/ui/job-context-selector.component';
import { JobSubmitService } from '../../../features/podium/job-submit/lib/job-submit.service';
import {
...
export class JobCreateComponent implements OnInit, OnDestroy {
  ...
  registeredConferenceIds = new Set<string>();
  ...
  private async initPage(): Promise<void> {
    ...
    await this.loadRegisteredConferenceIds();
  ...
  private async loadRegisteredConferenceIds(): Promise<void> {
    this.registeredConferenceIds = new Set<string>();
    try {
      const jobs = await this.httpService.getUserJobs(String(this.currentUser.id));
      jobs.forEach((job) => this.registeredConferenceIds.add(String(job.conferenceId)));
    } catch {
      /* ignore: job creation validation still exists on submit */
    }
    ...
    try {
      ...
      const prefix = `conference-registration:${this.currentUser.id}:`;
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key?.startsWith(prefix) && localStorage.getItem(key) === '1') {
          this.registeredConferenceIds.add(key.slice(prefix.length));
        }
      }
    } catch {
  ...
  private async loadJobForEdit(): Promise<boolean> {
    try {
      ...
      if (data.status === 'READY_FOR_PUBLICATION') {
        this.notificationService.showInfo('Работа', 'Работа со статусом «Готова к публикации» больше не редактируется.');
        await this.router.navigate([`/podium/job/${data.id}`]);
        return false;
      }
      ...
      this.breadcrumbItems = [
        { label: this.getShortJobTitle(), routerLink: `/podium/job/${this.currentJobId}` },
    ...
    if (this.isEditMode) {
      ...
      if (this.conferenceParamId) {
        ...
        if (this.currentConference) {
          if (!this.isConferenceRegistered(this.currentConferenceId)) {
            this.notificationService.showInfo('Конференция', 'Сначала зарегистрируйтесь на конференцию, затем подавайте работу.');
            void this.router.navigate([`/podium/conference/${this.currentConferenceId}`]);
            return;
          }
  ...
  private isConferenceRegistered(conferenceId: string): boolean {
    return this.registeredConferenceIds.has(String(conferenceId));
  }
