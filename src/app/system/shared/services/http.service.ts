...
export class HttpService {
  ...
  async login(request: object): Promise<LoginResponse> {
    return await firstValueFrom(this.http.post<LoginResponse>(`${this.baseUrl}/api/v1/auth/login`, request, this.httpOptions));
  }

  async logout(): Promise<any> {
    //todo не работает
    this.updateHeaders();
    return await firstValueFrom(this.http.post<any>(`${this.baseUrl}/api/v1/auth/logout`, this.httpOptions));
  }

  async registration(request: object): Promise<User> {
    return await firstValueFrom(this.http.post<User>(`${this.baseUrl}/api/v1/auth/registration`, request, this.httpOptions));
  }

  async getModerators(): Promise<UserBase[]> {
    this.updateHeaders();
    return await firstValueFrom(this.http.get<UserBase[]>(`${this.baseUrl}/api/v1/admin/user/getModerators`, this.httpOptions));
  }

  async getModeratorsByConference(conferenceId: string): Promise<UserBase[]> {
    this.updateHeaders();
    return await firstValueFrom(this.http.get<UserBase[]>(`${this.baseUrl}/api/v1/admin/conference/${conferenceId}/getModerators`, this.httpOptions));
  }

  async getReviewers(): Promise<UserBase[]> {
    this.updateHeaders();
    return await firstValueFrom(this.http.get<UserBase[]>(`${this.baseUrl}/api/v1/admin/user/getReviewers`, this.httpOptions));
  }
  ...
  async reviewJob(id: string, request: ReviewDto): Promise<void> {
    this.updateHeaders();
    return await firstValueFrom(this.http.post<void>(`${this.baseUrl}/api/v1/member/job/${id}/review`, request, this.httpOptions));
  }
  ...
  async getTags(): Promise<string[]> {
    this.updateHeaders();
    return await firstValueFrom(this.http.get<string[]>(`${this.baseUrl}/api/v1/member/conference/tags`, this.httpOptions));
  }
  ...
  async appointModeratorToConference(id: string, admins: UserBaseDto[]): Promise<void> {
    this.updateHeaders();
    return await firstValueFrom(this.http.post<void>(`${this.baseUrl}/api/v1/admin/conference/${id}/appointmoderator`, JSON.stringify(admins), this.httpOptions));
  }
