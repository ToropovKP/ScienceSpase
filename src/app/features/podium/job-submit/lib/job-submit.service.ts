...
export class JobSubmitService {
  ...
  async createJob(params: CreateJobParams): Promise<bigint | null> {
    ...
    try {
      ...
      if (error?.status === 400 && /not registered for conference/i.test(String(error?.error?.message ?? ''))) {
        this.notificationService.showWarning('Конференция', 'Сначала зарегистрируйтесь на конференцию, затем подавайте работу.');
      } else {
  ...
  async updateJob(params: UpdateJobParams): Promise<bigint | null> {
    ...
    try {
      ...
      if (error?.status === 409 || error?.status === 403) {
        this.notificationService.showWarning('Работа', 'Текущий статус работы не позволяет редактирование.');
      } else {
